...
class ColorizePipelineNode(VisualPipeline, CNode):
    ...
    def __init__(
        ...
        color: np.ndarray = np.zeros((FREQ_BINS, 3)),
        color_level: np.ndarray = np.array([1]),
        render: bool = True,
        alias: str | None = None,
    ) -> None:
        terminals = {
        ...
        self.color = Element(self, "color", ElementValue(color))
    ...
    def c_update(self):
        color_level = self.color_level.value[:, np.newaxis]
        self.output_rgb.value[:] = np.clip(
            self.input_rgb.value * (1 - color_level) + self.color.value * color_level,
            0,
            255,
        )
